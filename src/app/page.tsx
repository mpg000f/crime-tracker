...
interface ApiResponse {
  states: StateData[];
  national: { population: number; count: number; rate: number; pctChange: number };
  years: number[];
}
...
export default function Home() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/states?year=${year}&offense=${offense}`);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      {data && (
        <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
          <StatsCard
            ...
            value={formatNumber(data.national.count)}
            subtitle={`${formatRate(data.national.rate)} per 100k`}
            change={data.national.pctChange}
          />
          <StatsCard
            title="U.S. Population"
            value={formatNumber(data.national.population)}
          />
          <StatsCard
            title="Highest Rate"
            value={(() => {
              const top = [...data.states].sort((a, b) => b.rate - a.rate)[0];
          ...
          <StatsCard
            title="Lowest Rate"
            value={(() => {
              const bottom = [...data.states].sort((a, b) => a.rate - b.rate)[0];
      ...
      <div className="mb-8 rounded-lg border border-border bg-card p-4">
        {loading ? (
          ...
          <USChoropleth data={data.states} offenseLabel={offenseLabel} />
        ) : (
          <div className="flex h-96 items-center justify-center text-muted-foreground">
            Failed to load data
          </div>
      ...
      {data && (
        <div>
          ...
          <RankingTable data={data.states} offenseLabel={offenseLabel} />
        </div>
      )}
    </div>
  );
